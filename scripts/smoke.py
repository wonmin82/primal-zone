...
class Client:
    ...
    async def fight(self):
        ...
        await self.act("공격 어린청소룡", lambda state: state["encounter"] is not None)
...
async def main():
    ...
    try:
        ...
        await player.act("대화 윤대장", lambda state: "정비기록" in state["quest"])
        ...
        await player.act("착용 강철마체테", lambda state: state["attack"] > before_attack)
        ...
        await player.act("조사 정비기록", lambda state: "수리 발전기" in state["quest"])
