...
class Help(GameCommand):
    ...
    def run(self):
        self.caller.msg(
            ...
            "파티: 파티 · 플레이어 파티초대 · 파티수락 · 파티거절\n"
            "파티 관리: 플레이어 파티제외 · 플레이어 파티장위임 · 파티탈퇴 · 순번 파티분배\n"
            "전리품: 시체에서 모두 가져 · 시체에서 아이템 가져 · 모두 가져 · 아이템 가져\n\n"
            "첫 탐사: 윤대장 대화 → 북 → 어린청소룡 사냥 → 시체에서 모두 가져 → 가방\n"
            "전투 중 강타·방어·회복은 다음 차례에 실행됩니다. 반복 입력해도 공격 속도는 늘지 않습니다.\n"
            "일반 적은 탐사자/파티가 점유하고 우두머리는 공동 참여할 수 있습니다.\n"
            "경험치·크레딧은 참여자에게, 아이템은 시체에 배정됩니다. 시체 30초 · 보호 120초 · 재생성 45초.\n"
            "접속 종료 시 전투에서 이탈합니다. 재접속 후 살아 있는 적을 다시 지정하세요."
        )
