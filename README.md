...
Room은 눈앞의 대상과 실제 출구를 문장과 방향도로 보여줍니다. `<대상> 보기`로 설명과 가능한 행동을 확인하고, `도움말`에서 전체 명령을 찾을 수 있습니다. 소지한 장비도 `강철 마체테 보기`처럼 살펴볼 수 있습니다.

`상태`, `능력`, `경험치`, `기술`, `장비`, `가방`, `상점`, `임무`, `파티`는 정돈된 텍스트 정보창으로 표시합니다. 적·NPC·물건·명령·전리품은 의미별 색으로 구분하지만 로그의 단어는 클릭하는 링크가 아닙니다. 빠른 조작은 기존 SURROUNDINGS 버튼을 사용하세요. [색상과 출력 구조](docs/architecture.md#텍스트와-의미별-색상), [화면 검증 절차](docs/playtest.md#텍스트-인터페이스-확인)를 참고하세요.
