...
390px에서는 파티 패널·전리품 버튼·명령 입력을 스크롤로 이용할 수 있고 가로 넘침이 없어야 합니다. 글꼴은 Neo둥근모 Code로 표시되고 외부 CDN 요청이 없어야 합니다. 실제 OS 한글 IME 조합은 텍스트 붙여넣기나 가상 composition 이벤트와 별도로 검증합니다.
