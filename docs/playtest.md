...
| 2 | `대화 윤대장` | 통신탑 복구 임무를 받고 정비기록 조사 안내가 표시됩니다. |
| 3 | `북` | 바람 부는 초지로 이동하고 어린청소룡 사냥 버튼이 표시됩니다. |
| 4 | `공격 어린청소룡` | 교전이 시작되며 약 2.5초 간격으로 기본 공격과 적의 공격이 진행됩니다. |
...
부두에서 `북` → `공격 어린청소룡`으로 사냥하고, 전투가 끝난 뒤 `귀환` → `휴식`으로 회복합니다.
사냥 보상으로 경험치·크레딧과 회수부품을 모읍니다.

강철마체테를 얻으면 `착용 강철마체테`를 입력하고 `상태`에서 공격력이 증가했는지 확인합니다.
드롭을 기다리지 않고 부두의 상점·교환을 이용해 장비를 확보해도 됩니다.

| 물품 | 구매 명령과 가격 | 교환 명령과 재료 | 효과 |
| --- | --- | --- | --- |
| 강철마체테 | `구매 강철마체테`: 60 크레딧 | `교환 강철마체테`: 회수부품 6개 | 무기 공격 +6 |
| 강화조끼 | `구매 강화조끼`: 65 크레딧 | `교환 강화조끼`: 회수부품 6개 | 방어구 방어 +4 |
| 탐사카빈 | `구매 탐사카빈`: 130 크레딧 | `교환 탐사카빈`: 회수부품 12개 | 무기 공격 +10 |
| 붕대 | `구매 붕대`: 8 크레딧 | 교환 없음 | 1개로 체력 최대 35 회복 |

구매·교환은 한 번에 1개이며, 장비를 얻은 뒤 별도로 `착용 장비이름`을 입력합니다.
...
아래 경로는 부두에서 시작합니다. 아직 임무를 받지 않았다면 먼저 `대화 윤대장`을 입력합니다.

| 순서 | 입력 또는 조작 | 정상 결과 |
| --- | --- | --- |
| 1 | `북` → `동` | 초지를 거쳐 부서진 수송차에 도착합니다. |
| 2 | `조사 보급상자` | 붕대 2개를 받습니다. 이미 받은 계정은 추가로 받지 못합니다. |
| 3 | `서` → `북` → `동` | 초지, 발톱 자국 오솔길을 거쳐 폐쇄된 관리동에 도착합니다. |
| 4 | `조사 정비기록` | 발전기 수리 방법을 읽고 임무 안내가 바뀝니다. |
| 5 | `동` | 멈춰 선 발전실에 도착합니다. |
| 6 | 회수부품 3개 이상을 가진 상태에서 `수리 발전기` | 부품 3개가 소모되고 경험치 50을 받습니다. 통신탑 능선 진입이 열립니다. |
...
7. `귀환` → `대화 윤대장`으로 보고합니다. 추가로 경험치 100, 크레딧 100, 붕대 3개를 받습니다.
8. `임무`에 **통신탑 복구 완료**가 표시되는지 확인합니다. 다시 `대화 윤대장`을 입력해도 보상이 늘어나지 않아야 합니다.
...
| ID | 준비와 확인 방법 | 정상 결과 |
| --- | --- | --- |
| C-01 | 초지에서 어린청소룡과 교전 중 `공격 어린청소룡`을 짧게 몇 번 반복합니다. | 적 체력·전투 차례가 초기화되지 않고, 입력 횟수만큼 공격이 추가되지 않습니다. |
...
| C-13 | 장비와 체력을 준비한 뒤 오솔길에서 `공격 갈퀴사냥룡`, 발전실에서 `공격 고장난경비기`로 각각 승리합니다. | 갈퀴사냥룡은 경험치 38·크레딧 14, 경비기는 경험치 55·크레딧 20을 줍니다. 각각 부품 1개를 주고 강화조끼·탐사카빈은 확률로 나옵니다. |
...
| ID | 준비와 확인 방법 | 정상 결과 |
| --- | --- | --- |
| U-01 | 두 계정이 같은 장소에서 `말 테스트 메시지`를 입력합니다. | 같은 장소의 두 계정에 발신자 이름과 메시지가 표시됩니다. |
...
| `test` 두 번째 단계 | 월드 생성, DB 저장, 한국어 명령·임무, 인증 입력 처리 등 통합 검사 | `Ran 12 tests` 다음 `OK` |
...
2. 임무 수락, 초지 이동, 어린청소룡 6회 사냥과 보상
3. 사냥 사이 귀환·휴식, 강철마체테 확보·착용과 공격력 변화
4. 정비기록 조사 후 접속 종료·재접속과 진행 기록 유지
...
| 공격이 멈춰 있음 | `종료`·재접속 후에는 `공격`으로 교전을 재개해야 합니다. 전투 상대 선택 전에는 `공격 대상이름`이 필요합니다. |
